...
if "val clipboard = mutableListOf<LayerElement>()" not in content:
    content = content.replace('var isSingleImageSelected by mutableStateOf(false)', 'var isSingleImageSelected by mutableStateOf(false)\n    val clipboard = mutableListOf<LayerElement>()')
...
old_duplicate = '''    fun duplicateSelection() {
        ...
        performSnapshotAction("Duplicar Selección") {
            ...
            selectionManager.setSelection(duplicatedElements)
            
            // Entering transform mode automatically
            val newBounds = selectionManager.selectionBounds
            if (!newBounds.isEmpty) {
                currentSelectionMode = SelectionMode.TRANSFORM_BOX
                transformStartBounds.set(newBounds)
                transformCurrentMatrix.reset()
            }
        }
    }'''
