...
# Replace settings logic blocks
pattern_if = r'else if \(\w+\.registryId == "settings"\) \{\s*com\.sketcher\.sketchercompanionv1\.ui\.dialogs\.PersonalizationMenu\([\s\S]*?onShowIconEditor = \{[\s\S]*?\}\s*\)\s*\}'
...
# Remove the parameter in layouts
...
# Remove personalizationMenuContent parameter
content = re.sub(r'personalizationMenuContent:\s*\(\)\s*->\s*Unit,?', '', content)
content = re.sub(r'personalizationMenuContent\s*=\s*\{[\s\S]*?com\.sketcher\.sketchercompanionv1\.ui\.dialogs\.PersonalizationMenu\([\s\S]*?\}\s*\},?', '', content)
