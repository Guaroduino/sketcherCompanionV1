...
object StrokeGeometry {
     ...
     */
    fun isStrokeTouched(stroke: VectorStroke, x: Float, y: Float, tolerance: Float = 20f): Boolean {
        val inputs = stroke.points
        if (inputs.size < 2) return false

        // ConsiderStroke Width in Hit Test
        // Total Threshold = Eraser Radius (tolerance) + Stroke Radius (size / 2)
        // VectorStroke usually has variable width, use maxWidth or logic
        val strokeRadius = stroke.maxWidth / 2f
        val totalThreshold = tolerance + strokeRadius

        // 1. OPTIMIZACIÃ“N (Bounding Box):
        ...
        // 2. PRECISIÃ“N (Distancia a Segmentos):
    ...
    /**
     * MatemÃ¡tica: Calcula la distancia mÃ­nima de un punto (px, py) a un segmento de lÃ­nea (x1,y1)-(x2,y2).
     */
    private fun distanceToSegment(px: Float, py: Float, x1: Float, y1: Float, x2: Float, y2: Float): Float {
        ...
        // ProyecciÃ³n del punto sobre la lÃ­nea
