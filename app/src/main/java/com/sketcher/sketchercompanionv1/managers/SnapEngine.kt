...
object SnapEngine {
    ...
    fun getSnapPoints(layers: List<Layer>): List<SnapPoint> {
        ...
        for (stroke in visibleStrokes) {
            ...
            when (stroke.strokeType) {
                ...
                StrokeType.POLYLINE, StrokeType.FREEHAND, StrokeType.PEN, StrokeType.PAINT, StrokeType.PLUMA, StrokeType.WATERCOLOR -> {
    ...
    private fun getSegments(stroke: VectorStroke): List<Pair<PointF, PointF>> {
        ...
        if (stroke.strokeType == StrokeType.LINE || stroke.strokeType == StrokeType.POLYLINE || stroke.strokeType == StrokeType.FREEHAND || stroke.strokeType == StrokeType.PEN || stroke.strokeType == StrokeType.PAINT || stroke.strokeType == StrokeType.PLUMA || stroke.strokeType == StrokeType.WATERCOLOR) {
            for (i in 0 until pts.size - 1) {
